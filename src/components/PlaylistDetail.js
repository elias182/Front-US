...
const PlaylistDetail = ({ setCurrentSong }) => {
  ...
  useEffect(() => {
    ...
    fetch(`https://backend-us-production-8ae2.up.railway.app/api/listarep/${id}`)
      .then(response => {
        if (!response.ok) {
  ...
  const handleDeleteSong = async (songId) => {
    try {
      const response = await fetch(`https://backend-us-production-8ae2.up.railway.app/api/eliminarsong/${songId}`, {
        headers: {
      ...
      const updatedResponse = await fetch(`https://backend-us-production-8ae2.up.railway.app/api/listarep/${id}`);
